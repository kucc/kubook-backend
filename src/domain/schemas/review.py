...
class ReviewBase(CustomBaseModel):
    user_id: int = Field(..., title="user_id", description="유저 id", examples=1, ge=0)
    book_info_id: int = Field(..., title="book_info_id", description="책과 연결된 정보 id", examples=1, ge=0)
    review_content: str = Field(..., title="review_content", description="리뷰 사항", examples="")


class Review(ReviewBase):
    id: int = Field(..., title="review_id", description="책 리뷰 id", example=1, ge=0)
